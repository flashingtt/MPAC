...
from utils.utils import device, _get_clones
import math

import pdb


class CLIP4cirMaPLeFinal(nn.Module):
    def __init__(self, args):
        ...
        if args.fixed_image_encoder:
            if args.asynchronous:
                ...
                self.clip, self.fixed_image_encoder, self.clip_preprocess = clip.load(args, args.clip_model_name, args.design_details, device=device, jit=False)
        else:
            if args.asynchronous:
                self.clip, self.clip_image_encoder, self.clip_preprocess = \
                    clip.load(args, args.clip_model_name, args.design_details, device=device, jit=False)
            else:
                self.clip, self.clip_preprocess = clip.load(args, args.clip_model_name, args.design_details, device=device, jit=False)
        ...
        if args.combiner == 'combiner':
            ...
            self.combiner = FineGrainedWeightedSum(self.image_encoder.output_dim, args.projection_dim, args.hidden_dim)
...
class Sum(nn.Module):
    ...
    def forward(self, ref_imgfeats, text_feats):
        return F.normalize(ref_imgfeats + text_feats, dim=-1)
...
class MultiModalPromptLearner(nn.Module):
    ...
    def forward(self, text_inputs=None, fixed_imgfeats=None):
        ...
        if self.args.txt2img:
            ...
            return prompts, tokenized_prompts, self.proj(shared_ctx), self.compound_prompts_visual, text_deep_prompts   # pass here original, as for visual 768 is required
...
class FineGrainedWeightedSum(Combiner):
    ...
    def forward(self, image_features: torch.tensor, text_features: torch.tensor) -> torch.tensor:
        ...
        output = self.output_layer(combined_features) + \
            (dynamic_scalar + 0.1 * dynamic_vector) * text_features + \
                (1 - (dynamic_scalar + 0.1 * dynamic_vector)) * image_features
        return F.normalize(output, dim=-1)
...
class CombinerV3(Combiner):
    ...
    def forward(self, image_features: torch.tensor, text_features: torch.tensor) -> torch.tensor:
        ...
        output = self_attn_feats + (dynamic_scalar + 0.1 * dynamic_vector) * text_features + \
                (1 - (dynamic_scalar + 0.1 * dynamic_vector)) * image_features
        return F.normalize(output, dim=-1)
...
class CombinerV4(Combiner):
    ...
    def forward(self, image_features: torch.tensor, text_features: torch.tensor) -> torch.tensor:
        ...
        self_attn_feats = self.self_attn(cat_feats.unsqueeze(1)).squeeze(1)
        output = mod_imgfeats + self_attn_feats + (dynamic_scalar + 0.1 * dynamic_vector) * text_features + \
                (1 - (dynamic_scalar + 0.1 * dynamic_vector)) * image_features
        return F.normalize(output, dim=-1)
...
class CombinerV5(Combiner):
    def __init__(self, args, clip_feature_dim, projection_dim, hidden_dim):
        ...
        self.image_sa = SelfAttentionCell(args)
        self.text_sa = SelfAttentionCell(args)
    ...
    def forward(self, image_features: torch.tensor, text_features: torch.tensor) -> torch.tensor:
        ...
        text_sa = self.text_sa(text_features.unsqueeze(1)).squeeze(1)
        image_sa = self.image_sa(image_features.unsqueeze(1)).squeeze(1)

        text_projected_features = self.dropout1(F.relu(self.text_projection_layer(text_sa)))
        image_projected_features = self.dropout2(F.relu(self.image_projection_layer(image_sa)))
