...
class ResidualAttentionBlock_MaPLe(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, design_details=None,
        ...
        super(ResidualAttentionBlock_MaPLe, self).__init__()
            ...
            if len(compound_prompts_deeper) > 0:
                ...
                if not self.text_layer:
                    ...
                    if not (counter > len(compound_prompts_deeper) - 1):
                        # Remove the outputs produced by learnable tokens of previous layer
                        prefix = x[0:x.shape[0] - self.compound_prompt_nctx, :, :] # [197, B, 768]
                        # Create/configure learnable tokens of this layer
                        ...
                        # ****************************************************************************************
                        visual_context = visual_context.expand(x.shape[1], -1, -1).permute(1, 0, 2) # EXPAND HERE
                        # ****************************************************************************************
...
class OriginVisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int):
        ...
        self.transformer = OriginTransformer(width, layers, heads)
...
class VisionTransformer_MaPLe(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int,
                 design_details):
        super(VisionTransformer_MaPLe, self).__init__()
        self.input_resolution = input_resolution
        self.output_dim = output_dim
        self.conv1 = nn.Conv2d(in_channels=3, out_channels=width, kernel_size=patch_size, stride=patch_size, bias=False) # 把图片变成patch

        self.VPT_shallow = True

        scale = width ** -0.5 # patch一般是nxn的，这里的width表示有多少个patch：有width个patch
        self.class_embedding = nn.Parameter(scale * torch.randn(width))
        self.positional_embedding = nn.Parameter(scale * torch.randn((input_resolution // patch_size) ** 2 + 1, width))
        self.ln_pre = LayerNorm(width)
...
def build_model(args, state_dict: dict, design_details):
    ...
    if args.asynchronous:
        if args.clip_model_name != 'ViT-B/16':
            vision_heads = vision_width * 32 // 64
            clip_image_encoder = ModifiedResNet(layers=vision_layers,
                                                output_dim=embed_dim,
                                                heads=vision_heads,
                                                input_resolution=image_resolution,
                                                width=vision_width)
            ...
            vision_heads = vision_width // 64
            # if args.design_details['trainer'] == "MaPLe":
            #     clip_image_encoder = VisionTransformer_MaPLe(input_resolution=image_resolution,
            #                                                  patch_size=vision_patch_size,
            #                                                  width=vision_width,
            #                                                  layers=vision_layers,
            #                                                  heads=vision_heads,
            ...
            #                                                  design_details=design_details)
            # else:
            #     clip_image_encoder = VisionTransformer(input_resolution=image_resolution,
            #                                            patch_size=vision_patch_size,
            #                                            width=vision_width,
            #                                            layers=vision_layers,
            #                                            heads=vision_heads,
            #                                            output_dim=embed_dim,
            #                                            design_details=design_details)
            clip_image_encoder = OriginVisionTransformer(input_resolution=image_resolution,
                                                         patch_size=vision_patch_size,
                                                         width=vision_width,
                                                         layers=vision_layers,
                                                         heads=vision_heads,
        ...
        image_encoder_weights = {k: v for k, v in state_dict.items() if 'visual' in k}

        image_encoder_state_dict = {}
        for k, v in image_encoder_weights.items():
            image_encoder_state_dict[k.replace('visual.', '')] = v
    ...
    if args.fixed_image_encoder:
        vision_heads = vision_width // 64
        fixed_image_encoder = OriginVisionTransformer(
            input_resolution=image_resolution,
            patch_size=vision_patch_size,
            width=vision_width,
            layers=vision_layers,
    ...
    if args.fixed_image_encoder:
        if args.asynchronous:
            ...
            return model.eval(), fixed_image_encoder.eval()
    else:
        if args.asynchronous:
            return model.eval(), clip_image_encoder.eval()
        else:
            return model.eval()